...
.cran_packages <- c('tidyverse','patchwork','skimr', 'ipmr') 
...
write.csv(others, 
          paste0("adler_2007_ks/data/", sp_abb, "/other_pars_mean.csv"), 
          row.names = F)
...
pars  <- list(surv_b0 = extr_value(surv_out, "b0"),
...
# Dynamically add surv_b2, surv_b3, grow_b2, grow_b3 if they exist
for (coef_suffix in c("b2", "b3")) {
  surv_coef_suffix <- paste0("surv_", coef_suffix)
  grow_coef_suffix <- paste0("grow_", coef_suffix)
  
  surv_value <- extr_value(surv_out, surv_coef_suffix)
  if (!is.null(surv_value) && length(surv_value) > 0) {
    pars[[surv_coef_suffix]] <- surv_value
  }
  
  grow_value <- extr_value(grow_out, grow_coef_suffix)
  if (!is.null(grow_value) && length(grow_value) > 0) {
    pars[[grow_coef_suffix]] <- grow_value
  }
}
...
lam_out       <- data.frame(coefficient = names(lam_mean_ipmr), 
                            value       = lam_mean_ipmr)
write.csv(lam_out, 
          paste0("adler_2007_ks/data/", sp_abb, "/lambda_vec.csv"), 
          row.names = F)
