...
# anderson_2016_mt\data\pasm

## Read and clean the species data

getwd()
df <- read.csv("anderson_2016_mt/data/pasm/mt_pasm.csv", header = TRUE) %>% 
...
ggsave(paste0("anderson_2016_mt/results/pasm/inv_plot_per_year.png"),  
...
ggsave(paste0("anderson_2016_mt/results/pasm/overall_hist_sizes_log.png"), 
       plot = hist_sizes_log, 
...
plot_binned_prop <- function(df, bins, x, y) {
  library(dplyr)
  
  df %>%
    mutate(bin = cut({{x}}, breaks = bins)) %>%
    group_by(bin) %>%
    summarise(
      logsize_t0 = mean({{x}}, na.rm = TRUE),
      survives = mean({{y}}, na.rm = TRUE),
      n = n(),
      lwr = pmax(0, survives - 1.96 * sqrt((survives * (1 - survives)) / n)),
      upr = pmin(1, survives + 1.96 * sqrt((survives * (1 - survives)) / n))
    )
}
...
ggsave(paste0("anderson_2016_mt/results/pasm/overall_surv.png"), 
       plot = surv_overall, 
...
ggsave(paste0("anderson_2016_mt/results/pasm/overall_rec.png"), 
       plot = rec_overall, 
       width = 4, height = 3, units = "in", dpi = 150)
...
ggsave(paste0("anderson_2016_mt/results/pasm/gr_mod_bestfit_index.png"), 
...
ggsave(paste0("anderson_2016_mt/results/pasm/su_mod_bestfit_index.png"), 
