# Year-specific IPM with fire - Archbold - Chrysopsis highlandsensis
...
# Model structure --------------------------------------------------------------
# Survival, growth, flowering probability, and flowerhead number vary by year.
# Flowerhead-to-recruit recruitment may vary by recruit year (t1), but no site
# random effect is fitted or included in the IPM.
...
mods_re <- list(mod_re_0, mod_re_1, mod_re_2, mod_re_3)
mods_re_dAIC <- bbmle::AICctab(
  mods_re, weights = TRUE, sort = FALSE)$dAIC
...
# Site-specific point shapes are retained for observations only.
...
re_yr_plots <- function(i) {
  ...
  pred_year_re$pred_re <- predict_re_safe(
    model = mod_re_best,
    newdata = pred_year_re,
    re.form = NULL)
...
g_re_years <- wrap_plots(re_yrs) +
  ...
  plot_annotation(
    ...
    subtitle = paste(v_ggp_suffix, "- no site recruitment effect"),
    theme = theme(
      plot.title = element_text(size = 13, face = "bold"),
      plot.subtitle = element_text(size = 9)))
...
# Build parameter list for mean and year-specific IPMs
make_ipm_pars <- function(
    pars_mean, pars_year = NULL, pars_re_year_t1 = NULL,
    year = NULL, re_year_t1 = NULL) {
  pars <- pars_mean
  ...
  if (!is.null(year) && !is.null(pars_year)) {
    ...
    year_hits <- grep(
      paste0("_", year, "$"), names(pars_year), value = TRUE)
  ...
  # Recruitment random effects belong to the recruit census year t1.
...
lambda_year <- data.frame(
  ...
  lambda_no_fire = sapply(
    ipm_years, function(i) lambda_ipm_year(year = i, fire = 0)),
  lambda_fire    = sapply(
    ipm_years, function(i) lambda_ipm_year(year = i, fire = 1)))

lambda_year


# Observed PGR vs modeled projected lambda, site-paired ------------------------
# Sites retain their observed size distributions and fire states. Recruitment
# coefficients vary among years only and contain no site random effect.
