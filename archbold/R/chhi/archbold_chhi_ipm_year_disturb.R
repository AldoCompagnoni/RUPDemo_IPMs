...
prep_pars <- function(i,
  ...
  pars_year <- list(
    ...
    surv_b0 = extr_value_list(
      pars_var_wide,
      paste('surv_b0', yr_now, sep = '_')),
    
    grow_b0 = extr_value_list(
      pars_var_wide,
      paste('grow_b0', yr_now, sep = '_')),
    
    surv_bf = extr_value_list(pars_cons_wide, 'surv_bf'),
    grow_bf = extr_value_list(pars_cons_wide, 'grow_bf'),
  ...
  for (j in 1:num_surv_params) {
    ...
    value <- extr_value_list(
      pars_var_wide,
      paste(param_name, yr_now, sep = '_'))
    
    if (length(value) > 0) {
      
      pars_year[[param_name]] <- value
    }
  }
  
  # growth parameters
  for (j in 1:num_grow_params) {
...
# mean recruitment per year
emergence_rate <- df %>%
  group_by(year) %>%
  summarise(n = sum(recruit, na.rm = TRUE)) %>%
  summarise(mean_emergence = mean(n)) %>%
  pull(mean_emergence)

# Seed survival
seed_bank_size <- 1e6
P <- 200  # your assumption or estimate from flowering
seed_survival <- seed_bank_size / (seed_bank_size - emergence_rate + P)


constants$value[constants$coefficient == "seed_emergence_rate"] <- emergence_rate
constants$value[constants$coefficient == "seed_survival"] <- seed_survival
