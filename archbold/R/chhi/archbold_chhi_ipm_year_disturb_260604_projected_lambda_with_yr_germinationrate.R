...
df_ind <- df_og %>% 
  ...
  group_by(site, plant_id, year) %>% 
  summarise(
    fl_nr = max(hd, na.rm = TRUE), 
    recruit = max(recruit, na.rm = TRUE), .groups = "drop") %>% 
  mutate(fl_nr = if_else(is.infinite(fl_nr), 0, fl_nr)) %>% 
  left_join(
    df_fire %>% 
      transmute(site, year, 
                fire = if_else(fire == "burn", "Fire", "No fire")), 
    by = c("site", "year")) %>% 
  mutate(fire = replace_na(fire, "No fire"), fire = factor(fire))

site_year <- df_ind %>% 
  group_by(site, year) %>% 
  summarise(
    n_obs = n(), flowerheads = sum(fl_nr[recruit == 0], na.rm = TRUE), 
    recruits = sum(recruit == 1, na.rm = TRUE), 
    fire = if_else(any(fire == "Fire"), "Fire", "No fire"), .groups = "drop")

recruit_data <- site_year %>%
  transmute(site, year, recruits_t0 = recruits, n_obs_t0 = n_obs, fire_t0 = fire) %>%
  left_join(
    site_year %>%
      transmute(site, year = year + 1, flowerheads_tminus1 = flowerheads),
    by = c("site", "year")) %>%
  left_join(
    site_year %>% 
      transmute(site, year = year + 2, flowerheads_tminus2 = flowerheads),
    by = c("site", "year")) %>%
  filter(!is.na(flowerheads_tminus1), !is.na(flowerheads_tminus2)) %>%
  mutate(
    site = factor(site),
    fire_t0 = factor(fire_t0),
    log_fl_tminus1 = log1p(flowerheads_tminus1),
    log_fl_tminus2 = log1p(flowerheads_tminus2),
    z_fl_tminus1 = as.numeric(scale(log_fl_tminus1)),
    z_fl_tminus2 = as.numeric(scale(log_fl_tminus2)))

# negative binomial GLMM with site as random effect
mod_recr <- glmer.nb(
  recruits_t0 ~ 
    z_fl_tminus1 + z_fl_tminus2 + fire_t0 + offset(log(n_obs_t0)) + (1 | site),
  data = recruit_data, 
...
# simple plot
recruit_data %>%
  select(year, site, recruits_t0, fire_t0,
         log_fl_tminus1, log_fl_tminus2) %>%
  pivot_longer(
    cols = c(log_fl_tminus1, log_fl_tminus2),
    names_to = "predictor",
    values_to = "log_flowerheads") %>%
  mutate(
    predictor = recode(
      predictor,
      log_fl_tminus1 = "flowerheads t-1",
      log_fl_tminus2 = "flowerheads t-2")) %>%
  ggplot(aes(log_flowerheads, recruits_t0)) +
  geom_point(aes(shape = fire_t0), alpha = 0.7) +
  geom_smooth(
    method = "glm",
