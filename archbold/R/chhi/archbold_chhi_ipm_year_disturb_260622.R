...
# Seed production in t0 affect on recruits in t1 and t2 ------------------------
# data where response is recruits in year t0,
# predictors are flowerheads in t-1 and t-2
...
df_re <- df_site_year %>%
  transmute(site, year, re_t0 = re) %>%
  left_join(
    df_site_year %>%
      transmute(
        site, year = year + 1, fh_tminus1 = fh),
    by = c("site", "year")) %>%
  left_join(
    df_site_year %>%
      transmute(
        site, year = year + 2, fh_tminus2 = fh),
    by = c("site", "year")) %>%
  mutate(
    site = factor(site),
    re_t0_log = log1p(re_t0),
    fh_tminus1_log = log1p(fh_tminus1),
    fh_tminus2_log = log1p(fh_tminus2)) %>% 
  filter(!is.na(re_t0),
         !is.na(fh_tminus1_log),
         !is.na(fh_tminus2_log),
         !is.na(site))

mod_re_00 <- glm.nb(
  re_t0 ~ fh_tminus1_log,
  data = df_re)

mod_re_01 <- glm.nb(
  re_t0 ~ fh_tminus1_log + fh_tminus2_log,
  data = df_re)

mod_re_10 <- glmer.nb(
  re_t0 ~ fh_tminus1_log + (1 | site),
  data = df_re,
...
mod_re_11 <- glmer.nb(
  re_t0 ~ fh_tminus1_log + fh_tminus2_log + (1 | site),
  data = df_re,
  control = glmerControl(
    optimizer = "bobyqa", 
    optCtrl = list(maxfun = 2e5)))
...
mod_re_20 <- glmer.nb(
  re_t0 ~ fh_tminus1_log + (fh_tminus1_log | site),
  data = df_re,
  control = glmerControl(
    optimizer = "bobyqa", 
    optCtrl = list(maxfun = 2e5)))
...
mod_re_21 <- glmer.nb(
  re_t0 ~ fh_tminus1_log + fh_tminus2_log + (fh_tminus1_log | site),
  data = df_re,
  control = glmerControl(
    optimizer = "bobyqa", 
    optCtrl = list(maxfun = 2e5)))
...
mod_re_30 <- glmer.nb(
  re_t0 ~ fh_tminus1_log + (0 + fh_tminus1_log | site),
  data = df_re,
  control = glmerControl(
    optimizer = "bobyqa", 
    optCtrl = list(maxfun = 2e5)))
...
mod_re_31 <- glmer.nb(
  re_t0 ~ fh_tminus1_log + fh_tminus2_log + (0 + fh_tminus1_log | site),
  data = df_re,
  control = glmerControl(
    optimizer = "bobyqa", 
    optCtrl = list(maxfun = 2e5)))
...
mods_re <- list(mod_re_00, mod_re_01,
                mod_re_10, mod_re_11,
                mod_re_20, mod_re_21,
                mod_re_30, mod_re_31)
...
mod_re_ranef   <- coef(mod_re_best)$year


# simple plot
df_re %>%
  select(year, site, re_t0,
         fh_tminus1_log, fh_tminus2_log) %>%
  pivot_longer(
    cols = c(fh_tminus1_log, fh_tminus2_log),
    names_to = "predictor",
    values_to = "log_flowerheads") %>%
  mutate(
    predictor = recode(
      predictor,
      fh_tminus1_log = "flowerheads t-1",
      fh_tminus2_log = "flowerheads t-2")) %>%
  ggplot(aes(log_flowerheads, re_t0)) +
  ...
  labs(
    x = "log(1 + flowerheads)",
    y = "recruits in year t0",
    title = "Do flowerheads in previous years predict recruits at t0?")
...
# Recruit
# Helper functions for recruitment parameters
get_re_fixef <- function(mod) {
...
get_re_coef <- function(coefs, coef_name) {
  if(coef_name %in% names(coefs)) {
    out <- unname(coefs[coef_name])
  } else {
    out <- 0
  }
  return(out)
...
add_recruitment_pars <- function(pars, mod_re_best, recruit_data, site_ipm = NULL) {
  # Fixed effects
  re_fixef <- get_re_fixef(mod_re_best)
  pars$re_int        <- get_re_coef(re_fixef, "(Intercept)")
  pars$re_fh_tminus1 <- get_re_coef(re_fixef, "fh_tminus1_log")
  pars$re_fh_tminus2 <- get_re_coef(re_fixef, "fh_tminus2_log")
  # Population-level random effects by default
  pars$re_site_int        <- 0
  pars$re_site_fh_tminus1 <- 0
  # Reference value for t-2 flowerheads if the selected model contains t-2
  pars$fh_tminus2_ref_log <- mean(recruit_data$fh_tminus2_log, na.rm = TRUE)
  # Site-specific random effects
  if(!is.null(site_ipm) && inherits(mod_re_best, "merMod")) {
    re_ranef <- ranef(mod_re_best)$site
    site_ipm <- as.character(site_ipm)
    if(site_ipm %in% rownames(re_ranef)) {
      if("(Intercept)" %in% colnames(re_ranef)) {
        pars$re_site_int <- re_ranef[site_ipm, "(Intercept)"]
      }
      if("fh_tminus1_log" %in% colnames(re_ranef)) {
        pars$re_site_fh_tminus1 <- re_ranef[site_ipm, "fh_tminus1_log"]
      }
    }
  }
  return(pars)
...
# Update germination / emergence rate from observed recruits -------------------
#  yearly germination rate directly match the observed number of recruits 
#  recruits observed in year t + 1 were produced by the IPM transition from year t to t1

obs_recr_by_year <- df %>%
  group_by(obs_year = year) %>%
  summarise(
    obs_recruits = sum(recruit == 1, na.rm = TRUE),
    .groups = "drop") %>%
  mutate(
    kernel_year = obs_year - 1) %>%
  dplyr::select(kernel_year, obs_recruits) %>%
  right_join(
    tibble(kernel_year = years_v),
    by = "kernel_year") %>%
  mutate(
    obs_recruits = coalesce(obs_recruits, 0L),
    emerg_rate_obs = obs_recruits / seedbank_size)

# put yearly germination rates into the IPM parameters
for (i in seq_along(pars_yr)) {
  
  yr_now <- years_v[i]
  
  er_now <- obs_recr_by_year$emerg_rate_obs[
    match(yr_now, obs_recr_by_year$kernel_year)
  ]
  
  if (length(er_now) == 0 || is.na(er_now)) {
    er_now <- 0
  }
  
  pars_yr[[i]]$emerg_rate <- er_now
}

tibble(
  year = years_v,
  emerg_rate = sapply(pars_yr, function(p) p$emerg_rate),
  expected_recruits = emerg_rate * seedbank_size) %>%
  left_join(
    obs_recr_by_year %>%
      select(kernel_year, obs_recruits),
    by = c("year" = "kernel_year")) %>% 
  print(., n = Inf)
