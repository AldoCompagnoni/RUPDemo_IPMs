...
re_term_map <- list(
  re_b0 = c("(Intercept)"),
  re_b1 = c("fh_tminus1_log"),
  re_b2 = c("fh_tminus2_log"))
...
constants <- tibble::tribble(
  ...
  "mat_siz",              200,
  "fh_tminus1_ref",       mean(df_re$fh_tminus1, na.rm = TRUE),
  "fh_tminus1_ref_log",   log1p(mean(df_re$fh_tminus1, na.rm = TRUE)),
  "fh_tminus2_ref_log",   mean(df_re$fh_tminus2_log, na.rm = TRUE)) %>%
  mutate(
...
# Site-varying recruitment parameters -----------------------------------------

# These are only created if the selected recruitment model has site-level
# random effects. Otherwise this returns an empty list.

re_out_site <- extract_group_pars(
  model     = mod_re_best,
  group_var = "site",
  term_map  = re_term_map,
  fill_missing = FALSE)
...
# Optional combined parameter objects -----------------------------------------
...
if(is.null(pars_all_mean$fh_tminus1_ref)) {
  pars_all_mean$fh_tminus1_ref <- mean(df_re$fh_tminus1, na.rm = TRUE)
}

if(is.null(pars_all_mean$fh_tminus1_ref_log)) {
  pars_all_mean$fh_tminus1_ref_log <- log1p(pars_all_mean$fh_tminus1_ref)
}

if(is.null(pars_all_mean$fh_tminus2_ref_log)) {
  pars_all_mean$fh_tminus2_ref_log <- mean(df_re$fh_tminus2_log, na.rm = TRUE)
}
...
make_ipm_pars <- function(
    pars_mean, pars_year = NULL, pars_site = NULL, year = NULL, site = NULL) {
  ...
  # Add site-specific recruitment parameters
  # These are conditional coefficients from coef(model)$site,
  # so they replace re_b0, re_b1, and re_b2 directly.
  if(!is.null(site) && !is.null(pars_site)) {
    
    site <- as.character(site)
    
    site_hits <- grep( paste0("_", site, "$"), names(pars_site), value = TRUE)
    
    if(length(site_hits) > 0) {
      
      for(i in site_hits) {
        
        new_name <- sub( paste0("_", site, "$"), "", i)
        
        pars[[new_name]] <- pars_site[[i]]
      }
    }
  }
...
re_total_ref <- function(pars) {
  ...
  re_log <- get_par(pars, "re_b0", 0) +
    ...
    get_par(pars, "fh_tminus1_ref_log", 0) +
    get_par(pars, "re_b2", 0) *
    get_par(pars, "fh_tminus2_ref_log", 0)
...
rx <- function(x, pars, fire = 0) {
  ...
  re_per_fh <- re_ref /
    pmax(get_par(pars, "fh_tminus1_ref", 0), .Machine$double.eps)
...
lambda_year <- data.frame(
  ...
  lambda_no_fire = sapply(
    ipm_years, function(i) lambda_ipm_year(year = i, fire = 0, site = 0)),
  lambda_fire    = sapply(
    ipm_years, function(i) lambda_ipm_year(year = i, fire = 1, site = 0))
  )
