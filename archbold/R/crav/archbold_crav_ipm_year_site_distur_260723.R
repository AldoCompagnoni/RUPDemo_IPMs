# by year and site IPM with disturbance - Archbold - Menges 2016 - Crotalaria avonensis
...
v_mod_set_fr <- c()
v_mod_set_fr_n <- c()
v_mod_set_fr2re <- 3  # force year + site random slopes
...
# The slope can vary among recruitment years, sites, or both.
# No random site effect is used for survival, growth, or reproduction.
...
# Model indices use the same zero-based convention as the other vital rates:
# 0 = fixed, 1 = year, 2 = site, 3 = site + year.
...
mod_fr2re_fixef <- glmmTMB::fixef(mod_fr2re_best)$cond
mod_fr2re_ranef <- glmmTMB::ranef(mod_fr2re_best)$cond
...
make_fr2re_year_plot <- function(year_i) {
  ...
  pred_i <- expand_grid(
    ...
    mutate(
      ...
      year_t1 = factor(
        year_i, levels = levels(df_fr2re$year_t1)))
  ...
  pts_i <- df_i %>%
    mutate(bin = ntile(logfruits_t0, 8)) %>%
    group_by(bin, site) %>%
    summarise(
...
# 1. pars_mean: fixed effects and IPM constants.
# 2. pars_year: conditional year coefficients for plant vital rates and the
#    year-specific fruit-to-recruit slope with the site deviation set to zero.
# 3. pars_site_year_recruit: fixed + year + site fruit-to-recruit slopes.
...
get_group_ranef <- function(model, group_var) {
  if (inherits(model, 'glmmTMB')) {
    ranef_list <- glmmTMB::ranef(model)$cond
  } else if (inherits(model, 'merMod')) {
...
# Year-specific plant vital-rate parameters -----------------------------------
...
# Fruit-to-recruit random-slope deviations ------------------------------------
...
# Create mean, year-specific, or site-year parameter list ---------------------
make_ipm_pars <- function(year_i = NULL, site_i = NULL) {
  ...
  if (!is.null(site_i)) {
    if (is.null(year_i)) {
      stop('site_i requires year_i so the site-year slope can be selected')
    }
...
predict_recruits_from_fruits <- function(fruits, pars) {
  ...
  ifelse(fruits <= 0, 0, pmax(recruits, 0))
}
...
# Mean and mean-site year-specific IPMs ----------------------------------------
...
# Site-year projection flow ----------------------------------------------------
# 1. Select year-specific plant vital rates.
# 2. Add the site + year fruit-to-recruit slope.
# 3. Build the observed site-year size distribution.
# 4. Predict total site fruits under no-fire and fire states.
# 5. Convert the weighted site fruit total to recruits per fruit.
# 6. Mix no-fire and fire kernels using the proportion of quadrats burned.

# Proportion of sampled quadrats disturbed in each site-year ------------------
p_disturbance_site_year <- df %>%
  filter(
    ...
    !is.na(quad_id),
    !is.na(disturbance)) %>%
  distinct(site, year, quad_id, disturbance) %>%
  group_by(site, year) %>%
  summarise(
...
# Expected total fruits from the observed site-year size distribution --------
expected_site_year_fruits <- function(
    pars, site_i, year_i, disturbance = 0, df_init = df) {
  n <- as.integer(pars$mat_siz)
  L <- pars$L
  U <- pars$U
  h <- (U - L) / n
...
project_site_year_ipm <- function(
    site_i, year_i, p_disturbance_i, df_init = df) {
...
# Observed site-year counts, including sampled sites with zero sized plants ----
df_obs_pgr_site <- df_plot_year_sampled %>%
  transmute(
    site = factor(site),
...
df_proj_site <- df_site_year_projection_grid %>%
  mutate(
    projection = purrr::pmap(
      list(site, year, p_disturbance),
      function(site, year, p_disturbance) {
...
# Site-year observed and modeled growth ---------------------------------------
