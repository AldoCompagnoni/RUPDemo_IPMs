# IPM year-specific; removed small observations, historic, with disturbance
...
# Specification ---------------------------------------------------------------
# Bayesian model file suffix 
v_brm_suffix <- "shrink01_rebound5"
v_head <- c('archbold')
...
# Investigate suspicious small individuals -----------------------------------
small_cutoff <- 2
previous_large_cutoff <- 5

df_small_check <- df_ind %>%
  filter(state == 'active', size_t0 > 0) %>%
  arrange(id, year) %>%
  group_by(id) %>%
  mutate(
    size_prev = lag(size_t0),
    suspicious_small =
      size_t0 <= small_cutoff &
      size_prev >= previous_large_cutoff,
    shrink_ratio = size_t0 / size_prev) %>%
  ungroup()

# Suspicious observations
df_small_suspicious <- df_small_check %>%
  filter(suspicious_small) %>%
  arrange(size_t0)

df_small_suspicious %>%
  select(
    id, site, pop, qu, year, size_prev, size_t0,
    shrink_ratio, disturbance, disturbance_prev) %>%
  print(n = Inf)


# Full timelines of suspicious individuals ----------------------------------
small_suspicious_ids <- df_small_suspicious %>%
  distinct(id) %>%
  pull(id)

df_small_timeline <- df_ind %>%
  filter(
    id %in% small_suspicious_ids,
    state == 'active',
    size_t0 > 0) %>%
  mutate(
    suspicious_small = size_t0 <= small_cutoff &
      id %in% df_small_suspicious$id &
      paste(id, year) %in%
      paste(df_small_suspicious$id, df_small_suspicious$year))

fig_small_timeline <- ggplot(
  df_small_timeline,
  aes(year, size_t0, group = id)) +
  geom_line() +
  geom_point(size = 2) +
  geom_point(
    data = filter(df_small_timeline, suspicious_small),
    size = 3, color = 'red') +
  geom_hline(
    yintercept = small_cutoff,
    linetype = 'dashed',
    color = 'red') +
  facet_wrap(~ id, scales = 'free_y') +
  scale_x_continuous(breaks = sort(unique(df_small_timeline$year))) +
  labs(
    title = 'Size histories of suspicious small observations',
    subtitle = paste0(
      'Red = size <= ', small_cutoff,
      ' cm following size >= ', previous_large_cutoff, ' cm'),
    x = 'Year',
    y = 'Diameter') +
  theme_bw() +
  theme(
...
# Investigate suspicious observations <= 1 cm ---------------------------------
small_cutoff_1 <- 1
previous_large_cutoff_1 <- 5

df_small_check_1 <- df_ind %>%
  filter(state == 'active', size_t0 > 0) %>%
  arrange(id, year) %>%
  group_by(id) %>%
  mutate(
    size_prev = lag(size_t0),
    suspicious_small =
      size_t0 <= small_cutoff_1 &
      size_prev >= previous_large_cutoff_1,
    shrink_ratio = size_t0 / size_prev) %>%
  ungroup()

df_small_suspicious_1 <- df_small_check_1 %>%
  filter(suspicious_small) %>%
  arrange(size_t0)

df_small_suspicious_1 %>%
  select(
    id, site, pop, qu, year, size_prev, size_t0,
    shrink_ratio, disturbance, disturbance_prev) %>%
  print(n = Inf)
...
# Full timelines of suspicious individuals <= 1 cm ---------------------------
small_suspicious_ids_1 <- df_small_suspicious_1 %>%
  distinct(id) %>%
  pull(id)

df_small_timeline_1 <- df_ind %>%
  filter(
    id %in% small_suspicious_ids_1,
    state == 'active',
    size_t0 > 0) %>%
  mutate(
    suspicious_small =
      paste(id, year) %in%
      paste(df_small_suspicious_1$id, df_small_suspicious_1$year))

fig_small_timeline_1 <- ggplot(
  df_small_timeline_1,
  aes(year, size_t0, group = id)) +
  geom_line() +
  geom_point(size = 2) +
  geom_point(
    data = filter(df_small_timeline_1, suspicious_small),
    size = 3, color = 'red') +
  geom_hline(
    yintercept = small_cutoff_1,
    linetype = 'dashed', color = 'red') +
  facet_wrap(~ id, scales = 'free_y') +
  scale_x_continuous(breaks = sort(unique(df_small_timeline_1$year))) +
  labs(
    title = 'Size histories of suspicious observations <= 1 cm',
    subtitle = paste0(
      'Red = size <= ', small_cutoff_1,
      ' cm following size >= ', previous_large_cutoff_1, ' cm'),
    x = 'Year',
    y = 'Diameter') +
  theme_bw() +
  theme(
...
fig_small_timeline_1



# Investigate relative shrinkage ----------------------------------------------
df_shrink <- df_ind %>%
  filter(state == 'active', size_t0 > 0) %>%
  arrange(id, year) %>%
  group_by(id) %>%
  mutate(
    size_tm1 = lag(size_t0),
    size_ratio = size_t0 / size_tm1,
    shrinkage = 1 - size_ratio) %>%
  ungroup() %>%
  filter(
    is.finite(size_ratio),
    size_tm1 > 0)

summary(df_shrink$size_ratio)

ggplot(df_shrink, aes(size_ratio)) +
  geom_histogram(bins = 100) +
  geom_vline(xintercept = 0.1, linetype = 'dashed', color = 'red') +
  coord_cartesian(xlim = c(0, 1)) +
  theme_bw() +
  labs(
    x = expression(size[t0] / size[t-1]),
    y = 'Number of transitions')


df_shrink_extreme <- df_shrink %>%
  filter(size_ratio <= 0.1)


# Full timelines of extreme-shrinkage individuals ----------------------------
shrink_ratio_cutoff <- 0.1

shrink_ids <- df_shrink_extreme %>%
  distinct(id) %>%
  pull(id)

df_shrink_flag <- df_shrink %>%
  mutate(extreme_shrink = size_ratio <= shrink_ratio_cutoff) %>%
  select(id, year, size_ratio, shrinkage, extreme_shrink)

df_shrink_timeline <- df_ind %>%
  filter(
    id %in% shrink_ids,
    state == 'active',
    size_t0 > 0) %>%
  left_join(
    ...
    by = c("id", "year")) %>%
  mutate(
    extreme_shrink = replace_na(extreme_shrink, FALSE))

fig_shrink_timeline <- ggplot(
  df_shrink_timeline,
  aes(year, size_t0, group = id)) +
  geom_line() +
  geom_point(size = 2) +
  geom_point(
    data = filter(df_shrink_timeline, extreme_shrink),
    size = 3, color = "red") +
  facet_wrap(~ id, scales = "free_y") +
  scale_x_continuous(
    breaks = sort(unique(df_shrink_timeline$year))) +
  labs(
    title = "Size histories of extreme-shrinkage individuals",
    subtitle = paste0(
      "Red points = size[t0] / size[t-1] <= ", shrink_ratio_cutoff),
    x = "Year",
    y = "Diameter") +
  theme_bw() +
  theme(
...
# Remove implausible shrinkage-rebound size observations ----------------------
shrink_ratio_cutoff <- 0.1
rebound_ratio_cutoff <- 5
...
df_bad_size <- df %>%
  ...
  mutate(
    size_tm1 = lag(size_t0),
    size_ratio = size_t0 / size_tm1,
    rebound_ratio = size_t1 / size_t0) %>%
  ungroup() %>%
  filter(
    is.finite(size_ratio),
    size_ratio <= shrink_ratio_cutoff,
    survives == 1,
    size_t1 > 0,
    rebound_ratio >= rebound_ratio_cutoff) %>%
  distinct(id, year)

df_bad_size


# Remove bad measurement from both transitions -------------------------------
df <- df %>%
  left_join(
    df_bad_size %>%
      mutate(remove_size_t0 = TRUE),
    by = c("id", "year")) %>%
  left_join(
    df_bad_size %>%
      transmute(
    ...
    by = c("id", "year")) %>%
  mutate(
    remove_size_t0 = replace_na(remove_size_t0, FALSE),
    remove_size_t1 = replace_na(remove_size_t1, FALSE),
    size_t0 = if_else(remove_size_t0, NA_real_, size_t0),
    logsize_t0 = if_else(remove_size_t0, NA_real_, logsize_t0),
    logsize_t0_2 = if_else(remove_size_t0, NA_real_, logsize_t0_2),
    logsize_t0_3 = if_else(remove_size_t0, NA_real_, logsize_t0_3),
    size_t1 = if_else(remove_size_t1, NA_real_, size_t1),
    logsize_t1 = if_else(remove_size_t1, NA_real_, logsize_t1)) %>%
  select(-remove_size_t0, -remove_size_t1)


# Recreate working datasets ---------------------------------------------------
df_ind <- df %>%
  filter(row_type == "individual")
...
# AICc screening for LOO
loo_aicc_cutoff <- 6

if (length(v_mod_set_su) == 0) {
  keep_su_loo <- which(mods_su_dAICc <= loo_aicc_cutoff)
} else {
  keep_su_loo <- 2 * v_mod_set_su + c(1, 2)
}

tibble(
  model = names(mods_su),
  dAICc = as.numeric(mods_su_dAICc),
  run_loo = seq_along(mods_su) %in% keep_su_loo) %>%
  arrange(dAICc) %>%
  print(n = Inf)

# Bayesian fits for competitive models
mods_su_brms <- map(
  keep_su_loo,
  ~ brm(
    formula = formula(mods_su[[.x]]),
    data = df_su,
    family = bernoulli(),
    chains = 4,
    cores = 4,
    ...
    file = file.path(
      dir_result,
      paste0(
        "brm_su_", names(mods_su)[.x], "_", v_brm_suffix,
        "_4c_2000i_1000w")),
    file_refit = "on_change",
    refresh = 0))

names(mods_su_brms) <- names(mods_su)[keep_su_loo]
...
mods_su_loo_compare <- loo_compare(mods_su_loo)
...
# LOO model selection
mod_su_name_bestfit <- mods_su_loo_compare$model[1]

mod_su_index_bestfit <- match(
  mod_su_name_bestfit,
  names(mods_su))
...
if (length(v_mod_set_gr) == 0) {
  keep_gr_loo <- which(mods_gr_dAICc <= loo_aicc_cutoff)
} else {
  keep_gr_loo <- 2 * v_mod_set_gr + c(1, 2)
}

tibble(
  model = names(mods_gr),
  dAICc = as.numeric(mods_gr_dAICc),
  run_loo = seq_along(mods_gr) %in% keep_gr_loo) %>%
  arrange(dAICc) %>%
  print(n = Inf)
...
mods_gr_brms <- map(
  ...
  ~ brm(
    ...
    file = file.path(
      dir_result,
      paste0(
        "brm_gr_", names(mods_gr)[.x], "_", v_brm_suffix,
        "_4c_2000i_1000w")),
    file_refit = "on_change",
    refresh = 0))

names(mods_gr_brms) <- names(mods_gr)[keep_gr_loo]
...
mods_gr_loo_compare <- loo_compare(mods_gr_loo)
...
# LOO model selection
mod_gr_name_bestfit <- mods_gr_loo_compare$model[1]

mod_gr_index_bestfit <- match(
  mod_gr_name_bestfit,
  names(mods_gr))
...
summary(mods_gr_brms$gr_30)
summary(mods_gr_brms$gr_3)
...
if (length(v_mod_set_do) == 0) {
  keep_do_loo <- which(mods_do_dAICc <= loo_aicc_cutoff)
} else {
  keep_do_loo <- 2 * v_mod_set_do + c(1, 2)
}

tibble(
  model = names(mods_do),
  dAICc = as.numeric(mods_do_dAICc),
  run_loo = seq_along(mods_do) %in% keep_do_loo) %>%
  arrange(dAICc) %>%
  print(n = Inf)
...
mods_do_brms <- map(
  ...
  ~ brm(
    ...
    file = file.path(
      dir_result,
      paste0(
        "brm_do_", names(mods_do)[.x], "_", v_brm_suffix,
        "_4c_2000i_1000w")),
    file_refit = "on_change",
    refresh = 0))

names(mods_do_brms) <- names(mods_do)[keep_do_loo]
...
mods_do_loo_compare <- loo_compare(mods_do_loo)
...
# LOO model selection
mod_do_name_bestfit <- mods_do_loo_compare$model[1]

mod_do_index_bestfit <- match(
  mod_do_name_bestfit,
  names(mods_do))
...
mods_ra_loo_compare <- loo_compare(mods_ra_loo)
...
# LOO model selection
mod_ra_name_bestfit <- mods_ra_loo_compare$model[1]

mod_ra_index_bestfit <- match(
  mod_ra_name_bestfit,
  names(mods_ra))
...
if (length(v_mod_set_fl) == 0) {
  keep_fl_loo <- which(mods_fl_dAICc <= loo_aicc_cutoff)
} else {
  keep_fl_loo <- 2 * v_mod_set_fl + c(1, 2)
}

tibble(
  model = names(mods_fl),
  dAICc = as.numeric(mods_fl_dAICc),
  run_loo = seq_along(mods_fl) %in% keep_fl_loo) %>%
  arrange(dAICc) %>%
  print(n = Inf)
...
mods_fl_brms <- map(
  ...
  ~ brm(
    ...
    file = file.path(
      dir_result,
      paste0(
        "brm_fl_", names(mods_fl)[.x], "_", v_brm_suffix,
        "_4c_2000i_1000w")),
    file_refit = "on_change",
    refresh = 0))

names(mods_fl_brms) <- names(mods_fl)[keep_fl_loo]
...
if (length(mods_fl_loo) > 1) {
  mods_fl_loo_compare <- loo_compare(mods_fl_loo)
  mods_fl_loo_compare
  ...
  mod_fl_name_bestfit <- mods_fl_loo_compare$model[1]
} else {
  mods_fl_loo_compare <- NULL
  
  map(
    mods_fl_loo,
    pareto_k_table)
  
  mod_fl_name_bestfit <- names(mods_fl_loo)[1]
}

mod_fl_index_bestfit <- match(
  mod_fl_name_bestfit,
  names(mods_fl))
...
if (length(v_mod_set_fl_n) == 0) {
  keep_fl_n_loo <- which(mods_fl_n_dAICc <= loo_aicc_cutoff)
} else {
  keep_fl_n_loo <- 2 * v_mod_set_fl_n + c(1, 2)
}

tibble(
  model = names(mods_fl_n),
  dAICc = as.numeric(mods_fl_n_dAICc),
  run_loo = seq_along(mods_fl_n) %in% keep_fl_n_loo) %>%
  arrange(dAICc) %>%
  print(n = Inf)
...
mods_fl_n_brms <- map(
  ...
  ~ brm(
    ...
    file = file.path(
      dir_result,
      paste0(
        "brm_fl_n_", names(mods_fl_n)[.x], "_", v_brm_suffix,
        "_4c_2000i_1000w")),
    file_refit = "on_change",
    refresh = 0))

names(mods_fl_n_brms) <- names(mods_fl_n)[keep_fl_n_loo]
...
if (length(mods_fl_n_loo) > 1) {
  mods_fl_n_loo_compare <- loo_compare(mods_fl_n_loo)
  mods_fl_n_loo_compare
  
  map(
    mods_fl_n_loo,
    pareto_k_table)
  
  mod_fl_n_name_bestfit <- mods_fl_n_loo_compare$model[1]
} else {
  mods_fl_n_loo_compare <- NULL
  
  map(
    mods_fl_n_loo,
    pareto_k_table)
  
  mod_fl_n_name_bestfit <- names(mods_fl_n_loo)[1]
}


# LOO model selection
mod_fl_n_index_bestfit <- match(
  mod_fl_n_name_bestfit,
  names(mods_fl_n))
...
mods_re <- list(
  ...
  re_sc1 = mod_re_sc1,
  re_sc2 = mod_re_sc2,
  re_logsc0 = mod_re_logsc0,
  re_logsc1 = mod_re_logsc1,
  re_logsc2 = mod_re_logsc2,
  re_yrfire = mod_re_yrfire)
...
if (length(v_mod_set_re) == 0) {
  keep_re_loo <- which(mods_re_dAICc <= loo_aicc_cutoff)
} else {
  keep_re_loo <- v_mod_set_re + 1
}

tibble(
  model = names(mods_re),
  dAICc = as.numeric(mods_re_dAICc),
  run_loo = seq_along(mods_re) %in% keep_re_loo) %>%
  arrange(dAICc) %>%
  print(n = Inf)
...
mods_re_brms <- map(
  ...
  ~ brm(
    ...
    file = file.path(
      dir_result,
      paste0(
        "brm_re_", names(mods_re)[.x], "_", v_brm_suffix,
        "_4c_2000i_1000w")),
    file_refit = "on_change",
    refresh = 0))

names(mods_re_brms) <- names(mods_re)[keep_re_loo]
...
if (length(mods_re_loo) > 1) {
  mods_re_loo_compare <- loo_compare(mods_re_loo)
  mods_re_loo_compare
  ...
  mod_re_name_bestfit <- mods_re_loo_compare$model[1]
} else {
  mods_re_loo_compare <- NULL
  
  map(
    mods_re_loo,
    pareto_k_table)
  
  mod_re_name_bestfit <- names(mods_re_loo)[1]
}


# LOO model selection
mod_re_index_bestfit <- match(
  mod_re_name_bestfit,
  names(mods_re))
