...
# P. lewtonii has post-fire seedbank recruitment. Total flowering stems in
# year t predict recruits in year t + 1, with current fire as a fixed effect.
df_site_year_sampled <- df %>%
  distinct(site, year)
...
df_fs2r <- df %>%
  ...
  semi_join(
    df_site_year_sampled %>%
      transmute(site, year_t1 = year),
    by = c('site', 'year_t1')) %>%
  left_join(
    df %>%
      filter(recruits == 1) %>%
...
# Mean flowering-stem-to-recruit model ----------------------------------------
# Zero flowering stems predict zero recruits. No year or site random effects
# are fitted. The second candidate retains the species-specific fire effect.
mod_re_00 <- lm(
  re_t1_log ~ 0 + fs_t0_log,
  data = df_fs2r)
...
mods_re <- list(
  mod_re_00, mod_re_01, mod_re_10,
  mod_re_11, mod_re_12, mod_re_20)
...
pars_mean <- as.list(c(
  ...
  re_b0 = 0,
  re_b1 = extract_coef(mod_re_best, 'fs_t0_log'),
  re_bf = extract_coef(
    mod_re_best,
...
predict_recruits_from_stems <- function(
  ...
  ifelse(stems_nonnegative <= 0, 0, pmax(recruits, 0))
}
...
# Expected total flowering stems from the mean size distribution --------------
expected_mean_stems <- function(
    pars, disturbance_prev = 0, df_init = df) {
...
mean_stems_no_previous_fire <- expected_mean_stems(
  pars = pars_mean,
  disturbance_prev = 0)

mean_stems_previous_fire <- expected_mean_stems(
  pars = pars_mean,
  disturbance_prev = 1)


# Mean disturbance exposure ---------------------------------------------------
# Each sampled site-year receives equal weight. The four combinations preserve
...
df_disturbance_regime <- df %>%
  filter(
    ...
    !is.na(disturbance),
    !is.na(disturbance_prev)) %>%
  distinct(site, year, disturbance, disturbance_prev) %>%
  count(disturbance, disturbance_prev, name = 'n_site_years') %>%
  complete(
    disturbance = c(0, 1),
    disturbance_prev = c(0, 1),
    fill = list(n_site_years = 0)) %>%
  mutate(
    p_state = n_site_years / sum(n_site_years)) %>%
  arrange(disturbance, disturbance_prev)
...
# Only site transitions represented in consecutive years are retained.
df_counts_site_year <- df %>%
  ...
  ungroup() %>%
  filter(year_gap == 1, n_t0 > 0, n_t1 > 0)

df_obs_growth_year <- df_counts_site_year %>%
  group_by(year) %>%
  summarise(
