...
df <- read.csv(
  ...
  mutate(
    disturbance = as.factor(if_else(burn_mean > 0, 1, 0)),
...
df %>%
  ...
  summarise(
    ...
    nr_quad_below_equal3 = sum(n_individuals <= 3),
    prop_quad_below_equal3 = mean(n_individuals <= 3))
...
# Flower data ------------------------------------------------------------------
# Recruits are excluded from flowering probability and flower-number models.
df_fl <- df %>%
  filter(
    !is.na(flower),
    !is.na(logvol_t0),
...
mod_fl_1 <- glmer(
  flower ~ logvol_t0 + disturbance + (1 | year),
  data = df_fl, family = binomial, control = ctrl_glmer)
mod_fl_2 <- glmer(
  flower ~ logvol_t0 + logvol_t0_2 + disturbance + (1 | year),
  data = df_fl, family = binomial, control = ctrl_glmer)
mod_fl_3 <- glmer(
  flower ~ logvol_t0 + logvol_t0_2 + logvol_t0_3 + disturbance +
    (1 | year),
  data = df_fl, family = binomial, control = ctrl_glmer)
...
make_fl_year_plot <- function(year_i) {
  ...
  ggplot() +
    geom_point(
      data = pts_i,
      aes(logvol_t0, flower, color = disturbance),
      size = 1.1) +
    geom_line(
      data = pred_i,
      aes(logvol_t0, flower, color = disturbance),
...
mod_fl_n_0 <- glmer.nb(
  fl_nr ~ disturbance + (1 | year),
  data = df_fl_cond, control = ctrl_glmer)
mod_fl_n_1 <- glmer.nb(
  fl_nr ~ logvol_t0 + disturbance + (1 | year),
  data = df_fl_cond, control = ctrl_glmer)
mod_fl_n_2 <- glmer.nb(
  fl_nr ~ logvol_t0 + logvol_t0_2 + disturbance + (1 | year),
  data = df_fl_cond, control = ctrl_glmer)
# mod_fl_n_3 <- glmer.nb(
#   fl_nr ~ logvol_t0 + logvol_t0_2 + logvol_t0_3 + disturbance +
#     (1 | year),
#   data = df_fl_cond, control = ctrl_glmer)
...
make_fl_n_year_plot <- function(year_i) {
  ...
  ggplot() +
    geom_point(
      data = pts_i,
      aes(logvol_t0, fl_nr, color = disturbance),
      size = 1.1) +
    geom_line(
      data = pred_i,
      aes(logvol_t0, fl_nr, color = disturbance),
...
fl_term_map <- make_term_map('fl_', simple_size_term_map)
fln_term_map <- make_term_map('fln_', simple_size_term_map)
re_term_map <- list(
  re_b0 = c('(Intercept)'),
  re_b1 = c('fs_t0_log'),
...
fyx <- function(y, x, pars, h, disturbance = 0) {
  ...
  Rvec <- rx(x, pars, disturbance = disturbance)
  outer(Pvec, Rvec) * h
}
...
kernel <- function(pars, disturbance = 0) {
  ...
  F_ad <- fyx(y = y, x = y, pars = pars, h = h, disturbance = disturbance)
...
lambda_ipm <- function(pars, disturbance = 0) {
  Re(eigen(kernel(pars, disturbance = disturbance)$k_yx)$values[1])
}
...
lambda_ipm_year <- function(
  ...
  lambda_ipm(pars_i, disturbance = disturbance)
}
...
lambda_year <- data.frame(
  year = ipm_years,
  lambda_no_fire = sapply(ipm_years, function(i) {
...
fig_lambda_year <- lambda_year %>%
  pivot_longer(
    ...
    values_to = 'lambda') %>%
  mutate(
    disturbance = recode(disturbance,
                         lambda_no_fire = 'No fire', lambda_fire = 'Fire')) %>%
  ggplot(aes(x = year, y = lambda, color = disturbance)) +
...
fire_lookup_site <- df %>%
  ...
  summarise(
    disturbance = factor(
      ...
      levels = c(0, 1)),
    .groups = 'drop') %>%
  mutate(disturbance_num = as.numeric(as.character(disturbance)))

get_disturbance_site_y <- function(site_i, yr) {
  out <- fire_lookup_site %>%
    filter(site == site_i, year == yr) %>%
    pull(disturbance_num)
...
project_one_site_year <- function(yr, site_i) {
  ...
  pars_y <- make_ipm_pars(
    ...
    site = site_re,
    re_year_t1 = yr + 1)
  disturbance_y <- get_disturbance_site_y(site_i, yr)
  n_obs <- make_initial_n_site(year0 = yr, site_i = site_i, pars = pars_y)
  K <- kernel(pars = pars_y, disturbance = disturbance_y)$k_yx
  ...
  data.frame(
    ...
    proj_lambda = as.numeric((sum(n_proj) * h) / n_initial))
}
...
df_compare_site <- df_obs_pgr_site %>%
  left_join(
    df_proj_site,
    by = c('year', 'site', 'disturbance_num')) %>%
  mutate(
