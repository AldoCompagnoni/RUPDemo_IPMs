...
make_re_year_plot <- function(year_i) {
  df_i <- df_fs2r_previous_fire %>%
    filter(year_t1 == year_i) %>%
    mutate(
      fire_group = case_when(
        disturbance_t0 == '1' ~ 'Current-year fire',
        disturbance_prev == 1 ~ 'Previous-year fire',
        TRUE ~ 'No fire'))
  ...
  pred_i <- tidyr::expand_grid(
    ...
    mutate(
      ...
      line_group = case_when(
        disturbance_t0 == '0' ~ 'No current fire',
        disturbance_t0 == '1' ~ 'Current-year fire'))
  ...
  ggplot() +
    ...
    scale_color_manual(
      values = c(
        ...
        'No current fire' = 'black')) +
    scale_linetype_manual(
      values = c(
        'No current fire' = 'solid',
