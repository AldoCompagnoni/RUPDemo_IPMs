...
i1_hist <- ggplot(df %>% filter(recruit == 1), aes(x = size_t0)) +
  geom_histogram(binwidth = 1, fill = 'lightgray', color = 'black') +
  theme_minimal()

df_r <- df %>%
  filter(plant_id %in% df$plant_id[df$recruit == 1]) %>% 
  # 1_4_17 has a problem
  filter(!plant_id %in% c('1_4_17', '4_23_1', '4_24_15', '4_24_16', '4_24_2'))

mod_i1_glm_size_age_quad <- lmer(logsize_t0 ~ age + I(age^2) + (1| plant_id), data = df_r)
df_r$pred_glm_size_age_quad <- predict(mod_i1_glm_size_age_quad, newdata = df_r, re.form = NULL)

mod_i1_lm_size_age_quad  <- lm  (logsize_t0 ~ age + I(age^2), data = df_r)
df_r$pred_lm_size_age_quad <- predict(mod_i1_lm_size_age_quad, newdata = df_r, re.form = NULL)


i1_size_age <- ggplot(df_r, aes(x = age, y = logsize_t0)) +
...
i2_hist <- ggplot(df %>% filter(dormancy == 1), aes(x = size_t1)) +
  geom_histogram(binwidth = 1, fill = 'lightgray', color = 'black') +
  labs(title = 'Histogram of Size After Dormancy', x = 'Size', y = 'Frequency') +
  theme_minimal() +
  annotate('text', x = Inf, y = Inf, label = paste('n:', nrow(df %>% 
                                                                  filter(dormancy == 1) %>% 
...
for(i in 2:nrow(df)) {
  if(!is.na(df$dormancy[i]) && df$dormancy[i] == 1) {
    size_before_dormancy <- c(size_before_dormancy, df$size_t0[i - 1])
  }
}
...
print(df %>% 
  filter(plant_id == df %>% 
           ...
           select(plant_id) %>% 
           .[[1]]) %>% 
    select(year, plant_id, size_t0), n = 100)
...
for (i in 1:nrow(df_f)) {
  ...
  current_site <- df_f$site[i]
  current_quad <- df_f$quad_id[i]
  current_plant <- df_f$plant_id[i]
  
  # Subset the data for the current combination of site, quad, and plant
  group_data <- df_f %>%
    filter(site == current_site, quad_id == current_quad, plant_id == current_plant) %>%
  ...
  for (j in 1:nrow(group_data)) {
    if (!is.na(group_data$size_t0[j])) {
      ...
      df_f$age[df_f$site == current_site & 
                      df_f$quad_id == current_quad & 
                      df_f$plant_id == current_plant & 
                      df_f$year == group_data$year[j]] <- current_age
      # Increment the age for the next year
      current_age <- current_age + 1
    } else if (j > 1) {
      # If size_t0 is NA, carry the age from the previous row
      previous_year_age <- df_f$age[df_f$site == current_site &
                                           df_f$quad_id == current_quad &
                                           df_f$plant_id == current_plant &
                                           df_f$year == group_data$year[j - 1]]
      df_f$age[df_f$site == current_site & 
                      df_f$quad_id == current_quad & 
                      df_f$plant_id == current_plant & 
                      df_f$year == group_data$year[j]] <- previous_year_age
    }
  }
}
```
...
i3_hist <- ggplot(df_f %>% filter(age == 1), aes(x = size_t0)) +
  geom_histogram(binwidth = 1, fill = 'lightgray', color = 'black') +
  theme_minimal()
...
mod_flag_linear <- lmer(age ~ logsize_t0 + (1 | plant_id), data = df_f)
#summary(mod_flag_linear)

# Predict values using the linear model
df_f$predicted_linear <- predict(mod_flag_linear, newdata = df_f, re.form = NULL)

# Fit the second model: logsize_t0 ~ age + I(age^2) + (1 | plant_id) (quadratic model)
mod_flag_quad <- lmer(age ~ logsize_t0 + I(logsize_t0^2) + (1 | plant_id), data = df_f)
#summary(mod_flag_quad)

# Predict values using the quadratic model
df_f$predicted_quad <- predict(mod_flag_quad, newdata = df_f, re.form = NULL)

# Fit the third model: logsize_t0 ~ age + (1 | plant_id) (reversed model)
mod_flag_logsize_age <- lmer(logsize_t0 ~ age + (1 | plant_id), data = df_f)
#summary(mod_flag_logsize_age)

# Predict values using the logsize_t0 ~ age model
df_f$predicted_logsize_age <- predict(mod_flag_logsize_age, newdata = df_f, re.form = NULL)

# Fit the fourth model: logsize_t0 ~ age + I(age^2) + (1 | plant_id) (quadratic term for age)
mod_flag_quad_age <- lmer(logsize_t0 ~ age + I(age^2) + (1 | plant_id), data = df_f)
#summary(mod_flag_quad_age)

# Predict values using the logsize_t0 ~ age + I(age^2) model
df_f$predicted_quad_age <- predict(mod_flag_quad_age, newdata = df_f, re.form = NULL)



lm_flag_quad_age <- lm(logsize_t0 ~ age + I(age^2), data = df_f)
df_f$predicted_lm_quad_age <- predict(lm_flag_quad_age, newdata = df_f, re.form = NULL)
...
p_i3_mod_1 <- ggplot(df_f, aes(x = logsize_t0, y = age)) +
  geom_jitter(aes(color = plant_id), width = 0.1, height = 0.1, alpha = 0.6) +  
...
p_i3_mod_2 <- ggplot(df_f, aes(x = age, y = logsize_t0)) +
  geom_jitter(aes(color = plant_id), width = 0.1, height = 0.1, alpha = 0.6) +  
...
p_i3_mod_3 <- ggplot(df_f, aes(x = age, y = logsize_t0)) +
  geom_jitter(aes(color = plant_id), width = 0.1, height = 0.1, alpha = 0.6) +  
...
### Timing of the fire
```{r fi timing, message=FALSE, warning=FALSE}
df_fi_hit <- df_og %>% 
  select(
...
p_fire_suv
```
#### Fire on Survival models 

```{r fi su mods, message=FALSE, warning=FALSE}
...
```{r Fire on Flower the year after, message=FALSE, warning=FALSE}
df_fire_fl_0 <- df %>% 
  filter(!is.na(flower)) %>%
  filter(size_t0 != 0) %>%
...
Comparing years with and without fire events, a greater proportion of individuals flower during years when fire occurs

```{r Fire on Flowering on plant id level, message=FALSE, warning=FALSE}
df_fi_fl_p <- df %>% 
  group_by(site, quad_id, plant_id, year) %>% 
...
To explore how individual plant size influences flowering, we built a series of generalized linear models (GLMs) with a binomial error distribution. All models use log-transformed plant size as a predictor. We structured our modeling in three main stages, progressively adding complexity to capture both linear and non-linear effects of size, as well as its interaction with fire events.

In the first stage, we began with a simple model, excluding size but including fire. We then added size as a predictor along with its interaction with fire occurrence in the same year. To account for potential non-linear relationships, we extended this model by including a quadratic term for size and its interaction with fire, and finally a cubic term with its corresponding interaction. These models allowed us to examine how flowering probability changes with size under varying fire conditions within the same year.

In the second stage, we applied the same model structure but considered fire events from the previous year instead of the current one. This allowed us to test for potential lagged effects of fire on flowering.

Finally, in the third stage, we included both current and previous year fire events in the same model, along with their interaction.

```{r Fire on flowering models, message=FALSE, warning=FALSE}
# Intercept model
...
```{r Fire the year before on flowering models, message=FALSE, warning=FALSE}
# Intercept model
...
```{r Fire and fire the year before on flowering, message=FALSE, warning=FALSE}
# Quadratic model
...
##### Comparing the models using AIC and anova

To determine the best-fitting model, we used the Akaike Information Criterion (AIC) to compare all candidate models. This approach allowed us to identify the most informative model while balancing goodness of fit with model complexity. Once the top model was selected based on AIC, we conducted likelihood ratio tests (ANOVA) to compare it against other high-ranking models, evaluating whether additional terms significantly enhanced explanatory power.

Our analysis revealed that the model including plant size along with its quadratic term provided the best fit to the data. Although AIC scores suggested that fire in both the current and previous year contributes to explaining flowering probability, the effect of fire in the previous year was not statistically significant within the model. Furthermore, ANOVA results indicated that including the previous year's fire event did not lead to a significant improvement in model fit. These findings suggest that while recent fire history may have some relevance, the occurrence of fire in the same year plays a more dominant role in influencing flowering.

```{r Compare fire flowering models, message=FALSE, warning=FALSE}
mods_fi_fl      <- list(
...
mod_fi_fl_bestfit   <- mod_fi_fl_2.02
mod_fi_fl_ranef     <- coef(mod_fi_fl_bestfit)
```


Predicted probability of flowering as a function of log-transformed plant size, separated by fire event (fire vs. no fire). The plot shows model-based predictions (blue lines) with 95% confidence intervals (shaded areas) from the best-fitting generalized linear model, which includes plant size and its quadratic term. Raw flowering observations (0 or 1) are overlaid as jittered black points for visual reference. The left panel shows predicted flowering in years without fire, while the right panel shows predictions for years with fire. The results highlight an overall increase in flowering probability with plant size and demonstrate that flowering is generally more likely in years with fire, particularly for larger individuals.
...
Relationship between fruit production in consecutive years under fire and no-fire conditions.
This scatterplot compares individual fruit production - here max fruits in a year - in year t (x-axis) with year t+1 (y-axis), split by fire occurrence. Points represent individual plants, with red indicating years following a fire and black indicating years without fire. Linear regression lines illustrate trends for each group. A 1:1 reference line (dashed) is included.
...
```{r message=FALSE, warning=FALSE}
ggplot(data = df_fi_ftf_og %>% filter(ftf <=1)) +
  geom_histogram(aes(x = ftf)) + 
  facet_wrap('fire_event')
```
...
#### Missing data on flowering fix

As mentioned earlier, we found some individuals with more fruits than flowers. Currently, both values represent the month in each year when the individual showed its highest count (Still wonder if sum for flowers would not be a better approximation).
We made the following adjustments:
We removed zeros for individuals that didn’t flower, and when fruit counts were higher than flower counts, we adjusted the fruit number to match the flower count, assuming that some flowering events may have been missed.


```{r missing data on flowering, message=FALSE, warning=FALSE}
...
Looking at the percentages (excluding NAs), individuals with more fruits than flowers make up about 2% of the full dataset. When we exclude individuals that had neither flowers nor fruits, this number increases to around 7%. Finally, if we focus only on individuals that produced fruits, we find that 56.2% of them had more fruits than flowers.
...
Fruit-to-flower ratio in relation to plant size (log-transformed) at time t0, separated by fire event. Each point represents an individual that produced at least one flower. We looking only at individuals that did flower in that year. We do not find any fruiting in years of fires. (Can you maybe elaborate on why fire does not seem to have an effect on this vital rate)
