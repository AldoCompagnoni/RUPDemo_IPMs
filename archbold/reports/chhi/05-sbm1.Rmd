# 1st seedbank Model {#sb1_model}

## Overview


This document is a structured R Markdown version of the original year-specific IPM script for *Chrysopsis highlandsensis*. The code is kept in the same order as the script so that the workflow can be inspected from data loading through model fitting, IPM construction, and comparison of modeled and observed population growth.

**Note.** This is the legacy version of the workflow. It retains the older seed-bank constants (`seedbank_size <- 1e6`, `seed_input <- 200`) and the original model-selection/export structure. In the newer cleaned IPM script, the seed-bank state is formulated as an effective recruit-producing seed bank calibrated from observed recruitment and field germination.

The only mechanical edit made during conversion was to comment out an accidental standalone `z` line in the original script, which would otherwise stop knitting.


## Data


Read the raw demographic dataset and clean column names.
...
Read fire-treatment data and harmonize variable names for joining with the demographic dataset.
...
Collapse raw records to one row per site, plant, and year. This creates the core annual dataset used for survival, growth, flowering, recruitment, and IPM construction.
...
Prepare survival observations. Survival is binary and will be modeled using binomial GLMMs.


```{r 5 survival-data, error=TRUE}
# Survival data ----------------------------------------------------------------
df_su <- df %>% 
...
Fit candidate survival models of increasing size complexity and choose the best model by AIC. Year is included as a random intercept. The section also plots year-specific survival fits.


```{r 5 survival-model, error=TRUE}
# Survival model ---------------------------------------------------------------
# GLMM; binomial
# Intercept model
mod_su_0 <- glmer(
...
Prepare growth observations. Growth is modeled as log size at t+1 conditional on the plant being observed at t+1.


```{r 5 growth-data, error=TRUE}
# Growth data ------------------------------------------------------------------
df_gr <- df %>% 
  filter(size_t0 != 0) %>%
  filter(size_t1 != 0) %>%
...
Fit candidate growth models and choose the best model by AIC. Random year effects allow growth intercepts and slopes to vary among years. The section also plots year-specific growth fits.


```{r 5 growth-model, error=TRUE}
# Growth model -----------------------------------------------------------------
# Intercept model 
mod_gr_0 <- lmer(
...
Estimate the variance function for the growth kernel from squared residuals as a function of fitted growth.


```{r 5 growth-variance-year-specific, error=TRUE}
# Growth variance year specific ------------------------------------------------
...
Prepare binary flowering data and flower-head counts.


```{r 5 flower-data, error=TRUE}
# Flower data ------------------------------------------------------------------
df_fl <- df %>%
  filter(!is.na(flower)) %>%
...
Fit candidate flowering-probability models using binomial GLMMs and choose the best model by AIC.


```{r 5 flower-model, error=TRUE}
# Flower model -----------------------------------------------------------------
# GLMM; binomial
# Intercept model
mod_fl_0 <- glmer(
...
Fit negative-binomial flower-head-count models conditional on flowering and choose the best model by AIC.


```{r 5 flower-number-conditional-on-flowering, error=TRUE}
# Flower number conditional on flowering ---------------------------------------
df_fl_cond <- df_fl %>%
  filter(flower == 1) %>%
...
Estimate the size distribution of new recruits on the log-size scale.


```{r 5 recruit-size-distribution, error=TRUE}
...
Extract fixed, random, and constant parameter estimates into wide lists that can be passed to the custom IPM kernel functions.


```{r 5 exporting-parameter-estimates, error=TRUE}
# Exporting parameter estimates ------------------------------------------------
# Survival
...
Define the vital-rate functions and assemble the IPM kernel. The kernel contains a seed-bank state and a continuous aboveground size distribution.


```{r 5 building-the-year-specific-ipms-from-scratch, error=TRUE}
# Building the year-specific IPMs from scratch ---------------------------------
# Functions
# Inverse logit
...
Calculate mean asymptotic lambda under no-fire and fire conditions using the mean parameter set.


```{r 5 mean-population-growth-rate, error=TRUE}
# Mean population growth rate --------------------------------------------------
pars_mean <- pars_cons_wide
...
Prepare year-specific parameter lists by combining fixed constants with year-specific survival and growth coefficients.


```{r 5 population-growth-rates-for-each-year, error=TRUE}
# population growth rates for each year -------------------------------------
...
Create a year-level fire lookup used for year-specific IPM projection.


```{r 5 fire-vector, error=TRUE}
# fire vector ---------------------------------------------------------------
...
Calculate asymptotic lambda for each year using the year-specific kernel.


```{r 5 lambda-per-year, error=TRUE}
# lambda per year -----------------------------------------------------------
...
Calculate observed population growth rates from monitored counts and compare them against modeled asymptotic lambda.


```{r 5 observed-population-growth-rates, error=TRUE}
# observed population growth rates -----------------------------------------
...
# NOTE: removed accidental stray line: z
pop_counts %>%
  summarise(
...
Project the observed size distribution forward one year and compare observed population growth with both asymptotic lambda and projected lambda.


```{r 5 observed-pgr-vs-asymptotic-lambda-vs-projected-lambda, error=TRUE}
# Observed PGR vs asymptotic lambda vs projected lambda ------------------------
