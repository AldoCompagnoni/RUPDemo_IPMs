...
This chapter contains the latest year-specific IPM for *Chrysopsis highlandsensis*. It is a complete, self-contained IPM: it loads packages, reads and prepares the data, fits the vital-rate models, extracts fixed and random parameters, builds the IPM kernel, calculates mean and year-specific lambdas, and compares modeled one-year projections with observed population growth.

This model is the latest (July 3,2026) update on recruitment.
The current recruitment formulation is:

\[
\log(1 + R_{t+1}) = \beta_0 + \beta_1 \log(1 + FH_t) + \text{site effects} + \text{recruitment-year effects}.
\]

Thus, flowerhead (FH) production in year `t0` predicts recruits (R) observed in year `t1`. Recruitment random-year effects are indexed by the recruit census year (`year_t1`), while survival, growth, flowering, and flowerhead number are indexed by the projection year (`year`).

The fertility kernel does not contain a separate seed-bank state in this version. Instead, expected recruitment is scaled from the fitted site-year recruitment model and distributed across recruit sizes using the observed recruit size distribution.

## Setup, packages, and paths

This chunk contains the same setup used by the standalone IPM script. The model-set vectors allow either automatic AICc selection or manual selection by setting a model index.

```{r xnew-setup, message=FALSE, warning=FALSE}
# Setting the stage ------------------------------------------------------------
...
The data are reduced to one record per site--plant--year and then joined to the fire table. Size is represented as log rosette diameter.

```{r xnew-data, message=FALSE, warning=FALSE}
# Data -------------------------------------------------------------------------
...
Survival is fitted as a binomial GLMM with a year random intercept. The selected model is chosen by AICc unless a manual index is supplied.

```{r xnew-survival, message=FALSE, warning=FALSE, fig.width=12, fig.height=16, out.width="100%"}
# Survival data ----------------------------------------------------------------
df_su <- df %>% 
...
Growth is fitted as a Gaussian mixed model on log size. The growth models use maximum likelihood (`REML = FALSE`) because models with different fixed effects are compared by AIC.

```{r xnew-growth, message=FALSE, warning=FALSE, fig.width=12, fig.height=16, out.width="100%"}
# Growth data ------------------------------------------------------------------
df_gr <- df %>% 
  filter(size_t0 != 0) %>%
  filter(size_t1 != 0) %>%
...
## Flowering and flowerhead number

Flowering probability is binomial. Flowerhead number conditional on flowering is negative-binomial; non-integer flowerhead values are removed before fitting because the response must be an integer count.

```{r xnew-flowering, message=FALSE, warning=FALSE, fig.width=12, fig.height=16, out.width="100%"}
# Flowering data ---------------------------------------------------------------
df_fl <- df %>%
  mutate(
...
# Negative-binomial responses must be integer counts. Non-integer values are
# treated as data-entry/measurement issues and removed before fitting.
df_fl_cond_non_integer <- df_fl %>%
  filter(flower == 1) %>%
  filter(!is.na(fl_nr), fl_nr > 0) %>%
  filter(fl_nr != round(fl_nr))

df_fl_cond_non_integer
...
# Flowering probability plots: year-specific ------------------------------------

fl_yr_plots <- function(i) {
  
  year_i <- as.character(i)
  ...
  x_min <- min(fl_temp$logsize_t0, na.rm = TRUE)
  x_max <- max(fl_temp$logsize_t0, na.rm = TRUE)
  ...
  x_temp <- seq(x_min, x_max, length.out = 100)
  
  pred_no_fire <- data.frame(
    logsize_t0   = x_temp,
    logsize_t0_2 = x_temp^2,
    logsize_t0_3 = x_temp^3,
    fire         = factor("No fire", levels = levels(df_fl$fire)),
    year         = factor(year_i, levels = levels(df_fl$year)))
  
  pred_no_fire$flower <- predict(
    mod_fl_best,
    newdata = pred_no_fire,
    type = "response",
    re.form = NULL)
  ...
  fl_pts <- fl_temp %>%
    mutate(
      bin = if(length(unique(logsize_t0)) > 1) {
        cut(logsize_t0, breaks = 8)
      } else {
        factor("all")
      }) %>%
    group_by(bin, fire) %>%
  ...
  ggplot() +
    geom_point(
      data = fl_pts %>% filter(fire == "No fire"),
      aes(logsize_t0, flower),
      size = 1,
      color = "black") +
    geom_point(
      data = fl_pts %>% filter(fire == "Fire"),
      aes(logsize_t0, flower),
      size = 1,
      color = "red") +
    geom_line(
...
fl_yrs <- lapply(sort(unique(df_fl$year)), fl_yr_plots)

g_fl_years <- wrap_plots(fl_yrs) +
  plot_layout(ncol = 3) +
  plot_annotation(
...
# Flowerhead number plots: year-specific ----------------------------------------

fln_yr_plots <- function(i) {
  ...
  x_min <- min(fln_temp$logsize_t0, na.rm = TRUE)
  x_max <- max(fln_temp$logsize_t0, na.rm = TRUE)
  ...
  pred_no_fire <- data.frame(
    ...
    logsize_t0_2 = x_temp^2,
    logsize_t0_3 = x_temp^3,
    fire         = factor("No fire", levels = levels(df_fl_cond$fire)),
    year         = factor(year_i, levels = levels(df_fl_cond$year)))
  
  pred_no_fire$fl_nr <- predict(
    mod_fl_n_best,
    newdata = pred_no_fire,
    type = "response",
    re.form = NULL)
  ...
  ggplot() +
    geom_point(
      data = fln_pts %>% filter(fire == "No fire"),
      aes(logsize_t0, fl_nr),
      size = 1,
      color = "black") +
    geom_point(
      data = fln_pts %>% filter(fire == "Fire"),
      aes(logsize_t0, fl_nr),
      size = 1,
      color = "red") +
    geom_line(
    ...
    labs(
      title = year_i,
      x = expression('log(size)'[t0]),
      y = "Expected flowerhead number") +
    theme_bw() +
    theme(text = element_text(size = 6))
}

fln_yrs <- lapply(sort(unique(df_fl_cond$year)), fln_yr_plots)

g_fln_years <- wrap_plots(fln_yrs) +
  plot_layout(ncol = 3) +
  plot_annotation(
    title = "Flowerhead number - year specific",
    subtitle = v_ggp_suffix,
    theme = theme(
      plot.title = element_text(size = 13, face = "bold"),
      plot.subtitle = element_text(size = 9)))
...
## Recruitment: flowerheads at t0 to recruits at t1

Recruitment is fitted at the site-year level. The response is recruits observed in `t1`; the predictor is flowerheads produced in `t0`.

```{r xnew-recruitment, message=FALSE, warning=FALSE, fig.width=12, fig.height=16, out.width="100%"}
...
# Data are aggregated at the site-year level, so each site has only one point
# per recruitment year. We therefore do not draw site-specific lines within
# each year panel. The black line is the recruitment-year-specific trend if
# the selected model contains a year_t1 random effect; otherwise it is the
# fixed-effect trend.
...
get_year_coef_re <- function(model, year_i) {
  fe_re <- get_fixef_re(model)
  b0_re <- unname(fe_re["(Intercept)"])
  b1_re <- unname(fe_re["fh_t0_log"])
  
  if (inherits(model, "merMod") && "year_t1" %in% names(coef(model))) {
    coef_year_re <- coef(model)$year_t1
    year_i_chr <- as.character(year_i)
    
    if (year_i_chr %in% rownames(coef_year_re)) {
      if ("(Intercept)" %in% colnames(coef_year_re)) {
        b0_re <- coef_year_re[year_i_chr, "(Intercept)"]
      }
      if ("fh_t0_log" %in% colnames(coef_year_re)) {
        b1_re <- coef_year_re[year_i_chr, "fh_t0_log"]
      }
    }
  }
  
  return(c(b0_re = b0_re, b1_re = b1_re))
}
...
re_yr_plots <- function(i) {
  ...
  fe_re <- get_fixef_re(mod_re_best)
  
  pred_fixed_re <- data.frame(
    fh_t0_log = x_temp_re,
    pred_re = unname(fe_re["(Intercept)"]) +
      unname(fe_re["fh_t0_log"]) * x_temp_re)
  
  coef_year_i_re <- get_year_coef_re(
    model = mod_re_best,
    year_i = i)
  
  pred_year_re <- data.frame(
    fh_t0_log = x_temp_re,
    pred_re = coef_year_i_re["b0_re"] +
      coef_year_i_re["b1_re"] * x_temp_re)
...
Observed recruits provide the recruit size distribution used by the fertility kernel.

```{r xnew-recruit-size, message=FALSE, warning=FALSE}
# Recruit size distribution -----------------------------------------------
...
Fixed effects, year-specific coefficients, site recruitment deviations, and recruitment-year deviations are converted into named parameter lists used by the IPM.

```{r xnew-parameter-extraction, message=FALSE, warning=FALSE}
...
## IPM functions and kernel

This section defines the vital-rate functions, recruitment scaling, fertility matrix, growth-survival transition matrix, and the full IPM kernel.

```{r xnew-ipm-functions, message=FALSE, warning=FALSE}
...
make_ipm_pars <- function(
  ...
  # The extraction step stores deviations (re_u0_site_*, re_u1_site_*),
  # so these must be added to the fixed recruitment coefficients.
...
## Mean and year-specific lambda

Mean and year-specific lambdas are calculated from the dominant eigenvalue of the IPM kernel.

```{r xnew-lambda, message=FALSE, warning=FALSE}
...
First mean lambda without and then with fire
```{r xnew-lambda mean f01, message=FALSE, warning=FALSE}
lambda_ipm(pars_mean, fire = 0)
lambda_ipm(pars_mean, fire = 1)
```


```{r xnew-lambda yearspec, message=FALSE, warning=FALSE}
# Year-specific IPMs -----------------------------------------------------------
...
Specific examples, here for year 1999, with and without fire, at mean site and then at site = 1
```{r xnew-lambda yearspeci, message=FALSE, warning=FALSE}
...
```{r xnew-lambda year-specific, message=FALSE, warning=FALSE}
...
## Observed versus modeled population growth

Observed site-year size distributions are projected one year forward and compared with observed population growth.

```{r xnew-comparison, message=FALSE, warning=FALSE}
...
These checks summarize singular fits and convergence gradients for the mixed models.

```{r xnew-diagnostics, message=FALSE, warning=FALSE}
# Model diagnostics ------------------------------------------------------------
# These diagnostics identify singular fits and large convergence gradients for
# fitted mixed models. They do not change the IPM; they flag models that should
# be interpreted cautiously.

check_mer_model <- function(model) {
  ...
  data.frame(
    ...
    max_grad = ifelse(length(grad) > 0, max(abs(grad)), NA_real_))
}

mods_su_check <- bind_rows(lapply(mods_su, check_mer_model), .id = "model")
mods_gr_check <- bind_rows(lapply(mods_gr, check_mer_model), .id = "model")
mods_fl_check <- bind_rows(lapply(mods_fl, check_mer_model), .id = "model")
mods_fl_n_check <- bind_rows(lapply(mods_fl_n, check_mer_model), .id = "model")
mods_re_check <- bind_rows(lapply(mods_re, check_mer_model), .id = "model")
