...
The candidate set differs only in whether the intercept and/or slope are allowed to vary by site.

```{r by_site_fruit_to_recruit_models}
fr2re_site_mod_1 <- glmmTMB::glmmTMB(
...
fr2re_site_mod_2 <- glmmTMB::glmmTMB(
...
fr2re_site_mod_3 <- glmmTMB::glmmTMB(
...
fr2re_site_mod_4 <- glmmTMB::glmmTMB(
...
fr2re_site_mods_dAICc <- bbmle::AICctab(
  fr2re_site_mods,
  weights = TRUE,
  sort = FALSE
)$dAICc
...
The extraction below works for all four candidate models. If the selected model does not include a random intercept or random slope, that missing random effect is set to zero.

```{r by_site_fruit_to_recruit_coefficients}
fr2re_site_fixef <- glmmTMB::fixef(fr2re_site_mod_best)$cond

fr2re_site_ranef <- tryCatch(
  {
    glmmTMB::ranef(fr2re_site_mod_best)$cond$site %>%
      as.data.frame() %>%
...
df_fr2re_site_coef <- tibble(site = levels(df_fr2re_site$site)) %>%
  ...
  mutate(
    ...
    fr2re_b0_site = unname(fr2re_site_fixef["(Intercept)"]) + `(Intercept)`,
    fr2re_b1_site = unname(fr2re_site_fixef["logfruits_site_t0"]) + logfruits_site_t0
  ) %>%
