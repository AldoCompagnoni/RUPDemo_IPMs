# Yearly IPM; w.out shrink obs BRMS {#ipm-yr_woshrinkobs}

## Yearly IPM with size-observation cleaning

Section 7 follows the same yearly IPM approach as Section 6, but adds a cleaning step for implausible size observations before fitting the vital-rate models.

Individuals showing an extreme size decrease (`size[t0] / size[t-1] <= 0.1`) followed by a strong rebound (`size[t1] / size[t0] >= 5`) are identified. The suspicious size measurement is set to missing for both transitions in which it occurs, while the remaining observations for that individual are retained.

After this cleaning step, the individual and recruitment datasets are rebuilt and the same model-selection and IPM workflow as in Section 6 is rerun using the cleaned data.
