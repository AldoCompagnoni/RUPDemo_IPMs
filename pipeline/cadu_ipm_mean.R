# IPM for Anderson 2016; Montana; Carex duriuscula 
...
k_matrix <- kernel(pars)$k_yx
print(k_matrix)

sum(is.nan(k_matrix))  # Count the number of NaN values
any(is.na(k_matrix))   # Check if there are any NA values
str(k_matrix)          # Check the structure of the matrix
...
#Error in eigen(kernel(i)$k_yx) : infinite or missing values in 'x'
lam_mean
