...
# patchwork, skimr, lme4, ggthemes, boot, 
...
ggsave(paste0(dir_result, '/0_age_surv.png'),  
       plot = fig_age_surv,
       width = 6, height = 9, dpi = 150)


# Survival ---------------------------------------------------------------------
v_age_surv_threshold <- if (
  !exists("v_age_surv_threshold")) 1 else v_age_surv_threshold

df_su_mod <- df_suv %>%
  mutate(age = replace(age, age > v_age_surv_threshold, v_age_surv_threshold))

# Survival model: GLM
mod_surv   <- glm(survives ~ age,
                  data = df_su_mod,
                  family = 'binomial')
...
# Check if v_age_surv_threshold is already defined (manual specification), 
...
mod_surv   <- glmer(survives ~ age + (1 | year) + (1 | quad),
                    data = mutate(df_su_mod, age = as.factor(age)),
                    family = 'binomial')

# calculate year-specific mean predictions
surv_ysmpred_re_fun <- function(mod, re) {
  shat_df       <- expand.grid(age  = as.factor(c(0:v_age_surv_threshold)),
...
pcr_df       <- df_rec %>% subset(!(recruits == 0))
