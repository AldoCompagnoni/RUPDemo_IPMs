...
# load_packages(patchwork, skimr, lme4, ggthemes, boot)
...
# Save all data ----------------------------------------------------------------
write.csv(df,     row.names = F, paste0(dir_data, '/data_df.csv'))
write.csv(df_suv, row.names = F, paste0(dir_data, '/survival_df.csv'))
write.csv(df_rec, row.names = F, paste0(dir_data, '/recruitment_df.csv'))
...
mod_su_prob <- data.frame(
  age     = sort(unique(df_su_mod$age)), 
  pred_su = predict(
    mod_su, newdata = data.frame(age = sort(unique(df_su_mod$age))), 
    type = "response"))
...
fig_su <- ggplot(data = df_su_mod %>% filter(!is.na(age)), 
  ...
  geom_segment(data = mod_su_prob %>% mutate(age = age + 1),
               aes(x = age - 0.25, 
                   xend = age + 0.25, 
                   y = pred_su, 
                   yend = pred_su),
  ...
  theme_minimal() +
  labs(title = 'Survival by age class',
       subtitle = v_fig_subt,
       y = 'Survival to t1',
       x = 'Age class')

ggsave(file.path(dir_result, '1_survival_mean.png'),  
...
fig_re <- ggplot(data = df_rec, aes(x = parents, y = recruits)) + 
  ...
  geom_line(aes(y = df_rec$parents + df_re_pred$pred[4]), 
            color = 'blue') + 
  theme_minimal() + 
  labs(title    = 'Recruitment with prediction',
       subtitle = v_fig_subt,
       x        = 'Nr. of individuals in t0',
       y        = 'Recruits in t1')

ggsave(file.path(dir_result, '1_recruitment.png'),  
...
df_lam <- df_mpm %>% 
  mutate(lam      = year_lam(df_mpm),
         proj_lam = proj_lam(df_mpm))
