...
server <- function(input, output, session) {
  ...
  output$plt <- renderPlotly({
    ...
    g <- dat %>%
      ...
      ggplot(aes(
        ...
        y = logsize_t1,
        text = tooltip,
        key = id_quad_year  # important for reliable selection
      )) +
      geom_point(
        alpha = 0.7,
        pch = 16,
  ...
  row_r <- eventReactive(event_data("plotly_click"), {
    ...
    req(click)
    id <- click$key
    out <- grow_df_filtered() %>% filter(id_quad_year == id)
  ...
  observeEvent(input$upload_excluded, {
    ...
    tryCatch({
      new_ids <- readr::read_csv(input$upload_excluded$datapath, show_col_types = FALSE)$id_quad_year
      excluded_ids(unique(c(excluded_ids(), new_ids)))  # merge
    }, error = function(e) {
      showNotification("Error reading CSV. Ensure it has a column named 'id_quad_year'.", type = "error")
    })
  })
}
