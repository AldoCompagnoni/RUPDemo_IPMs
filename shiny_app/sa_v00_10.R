...
# Version 2: Growth plot + exclusions + upload/download
...
server <- function(input, output, session) {
  ...
  row_r <- eventReactive(event_data("plotly_click"), {
    ...
    req(click)
    out <- grow_df_filtered()[click$pointNumber + 1, c("track_id", "quad", "year")]
    clipr::write_clip(out)  # copy to clipboard
    out
  })
  
  output$clicked_id <- renderPrint(row_r())
