...
ui <- fluidPage(
  fluidRow(
    column(
      ...
      selectizeInput(
        "exclude_input",
        "Exclude point (id–quad–year):",
        choices = c("", sort(unique(grow_df$id_quad_year))),
        selected = "",
        multiple = FALSE,
        options = list(
        ...
        tags$div(
          ...
          tags$h5(
            ...
            tags$a(
              ...
              tags$small("Excluded points (id–quad–year)")
            )
          )
        ),
        tags$div(
...
server <- function(input, output, session) {
  ...
  output$excluded_list <- renderUI({
    ...
    tagList(
      lapply(ids, function(id) {
        fluidRow(
          column(8, id, style = "font-size: 0.85em;"),
  ...
  row_r <- eventReactive(event_data("plotly_click"), {
    ...
    out <- grow_df_filtered()[click$pointNumber + 1,
                              c("track_id", "quad", "year", "id_quad_year", "logsize_t0", "logsize_t1")]
