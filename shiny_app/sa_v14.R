...
# Version 14: Misfits list with two-row layout
...
df <- readRDS(file.path(
  ...
  select(-c(suspect, near_edge)) %>%                                   # keep site (we’ll re-derive)
  ...
  mutate(
    ...
    # site from track_id; double-backslash so it’s literal in R source
    site = sub("^.{3}_([A-Za-z]+)_.*$", "\\\\1", track_id),
...
server <- function(input, output, session) {
  ...
  # ---- Capture clicked point (copy small summary to clipboard) ----
